...
// Matches the continuation stub length drawn by MapCanvas.
export const CONTINUE_ROOM = 130;
export const R = 11;
...
export function buildLayout(mapData) {
  ...
  /**
   * Lanes are assigned per orientation, so a horizontal and a vertical street
   * can share a lane number without landing on top of one another.
   *
   * Explicit lanes are honoured first and claimed, then the rest fill the gaps
   * left over. Handing out a number that another street already holds would
   * draw the two lines on top of each other.
   */
  const laneOf = new Map();
  const taken = { h: new Set(), v: new Set() };

  for (const line of mapData.lines) {
    const explicit = Number.isFinite(line.lane) && line.lane > 0 ? line.lane : null;
    if (explicit !== null && !taken[line.orientation].has(explicit)) {
      laneOf.set(String(line._id), explicit);
      taken[line.orientation].add(explicit);
    }
  }

  for (const line of mapData.lines) {
    if (laneOf.has(String(line._id))) continue;
    const used = taken[line.orientation];
    let n = 0;
    while (used.has(n)) n += 1;
    laneOf.set(String(line._id), n);
    used.add(n);
  }
  ...
  // Continuation stubs run past the last station, so the canvas has to leave
  // room for them or they get clipped at the edge.
  const tail = lines.some((l) =>
    l.stations.some((s) => s.continues && s.continues !== 'none')
  )
    ? CONTINUE_ROOM
    : 0;
  ...
  return {
    lines,
    width: Math.max(720, (xs.length ? Math.max(...xs) : 0) + PAD + 60 + tail),
    height: Math.max(380, (ys.length ? Math.max(...ys) : 0) + PAD + tail),
  };
}
